...
class ManTau(scrapy.Spider):
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        super(ManTau, self).__init__(*args, **kwargs)

        keywords = [keyword[:keyword.index('*')] for keyword in self.config['keywords'].keys()
                                               if '*' in keyword]
        self.url = 'https://man-tau.com/page/{}/?s={}'
    ...
    def parse_news_page(self, response):
        ...
        author = response.css('span.vcard.author>*>a::text').get()
        provenance = response.url
        last_modified = response.css('meta[property="article:modified_time"]::attr(content)').get()
        all_para_tags  = response.css('div.entry-content>p::text').getall()
    ...
    def parse(self, response):
        ...
        links_in_page =  response.css('div.meta-image>a::attr(href)').getall()
        next_page = response.css('nav.vce-load-more>a::attr(href)').get()
