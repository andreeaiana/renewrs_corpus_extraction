...
class ContraMagazin(scrapy.Spider):
    name = "contra_magazin"
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        super(ContraMagazin, self).__init__(*args, **kwargs)

        keywords = [keyword[:keyword.index('*')] for keyword in self.config['keywords'].keys()
                                               if '*' in keyword]
        self.url = 'https://www.contra-magazin.com/page/{}/?s={}'
    ...
    def parse_news_page(self, response):
        ...
        all_para_tags  = p = response.css("div.entry-content.clearfix>p::text").getall()
