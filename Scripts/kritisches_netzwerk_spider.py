...
class Kritisches_netzwerk(scrapy.Spider):
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        super(Kritisches_netzwerk, self).__init__(*args, **kwargs)

        keywords = [keyword[:keyword.index('*')] for keyword in self.config['keywords'].keys()
                                               if '*' in keyword]
        self.url = 'https://kritisches-netzwerk.de/search/node/{}?page={}'
        self.url_vs_query = {self.url.format(urllib.parse.quote(query), '0'):query for query in keywords}
    ...
    def parse_news_page(self, response):
        ...
        date_published = response.css('meta[property="article:published_time"]::attr(content)').get()
        ...
        author = response.css('meta[name="dcterms.creator"]::attr(content)').get().strip()
        provenance = response.url
        last_modified = response.css('meta[property="article:modified_time"]::attr(content)').get()
        all_para_tags  =   response.css('div.field-item.even>p::text').getall()
    ...
    def parse(self, response):
        if self.article_num > self.config["top_k"]:
            ...
            return
        parsed_url = parse.urlsplit(response.url).path
        links_in_page =  response.css('h3.title>a::attr(href)').getall()
        next_page = response.css("li.next>a::attr(href)").get()
        for link in links_in_page:
            yield scrapy.Request(link, callback = self.parse_news_page,
            meta = {'query':parsed_url[parsed_url.rfind('/')+1:]})
