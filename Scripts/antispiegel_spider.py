...
class Antispiegel(scrapy.Spider):
    name = "antispiegel"
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        super(Antispiegel, self).__init__(*args, **kwargs)

        keywords = [keyword[:keyword.index('*')] for keyword in self.config['keywords'].keys()
                                               if '*' in keyword]
        self.url = 'https://www.anti-spiegel.ru/page/{}/?s={}'
    ...
    def parse_news_page(self, response):
        ...
        author = response.css('h2.author-title::text').get().strip()
    ...
    def parse(self, response):
        ...
        links_in_page =  response.css("h2.entry-title>a::attr(href)").getall()
