...
class Journalistenwatch(scrapy.Spider):
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        super(Journalistenwatch, self).__init__(*args, **kwargs)

        keywords = [keyword[:keyword.index('*')] for keyword in self.config['keywords'].keys()
                                               if '*' in keyword]
        self.url = 'https://www.journalistenwatch.com/page/{}/?s={}'
    ...
    def parse_news_page(self, response):
        ...
        author = response.css("meta[itemprop=author]::attr(content)").get()
        provenance = response.url
        last_modified = response.css('meta[property="article:modified_time"]::attr(content)').get()
        all_para_tags  = p = response.css("div.td-post-content>p::text").getall()
