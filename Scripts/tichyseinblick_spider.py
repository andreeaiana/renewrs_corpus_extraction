...
class Tichyseinblick(scrapy.Spider):
    name = "tichyseinblick"
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        super(Tichyseinblick, self).__init__(*args, **kwargs)

        keywords = [keyword[:keyword.index('*')] for keyword in self.config['keywords'].keys()
                                               if '*' in keyword]
        self.url = 'https://www.tichyseinblick.de/page/{}/?s={}'
    ...
    def parse_news_page(self, response):
        ...
        author = response.css('div.rty-article-page-author>*>a::text').get()
        provenance = response.url
        last_modified = response.css('meta[property="article:modified_time"]::attr(content)').get()
        all_para_tags  = response.css('div.pf-content>p').getall()
    ...
    def parse(self, response):
        ...
        links_in_page =  response.css('div.category-content-title>a::attr(href)').getall()
