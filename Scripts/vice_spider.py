...
class Vice(scrapy.Spider):
    ...
    def __init__(self, config=None, *args, **kwargs):
        ...
        keywords = [keyword[:keyword.index('*')] for keyword in self.config['keywords'].keys()
                                               if '*' in keyword]
        self.url = 'https://www.vice.com/de/search?page={}&q={}'
    ...
    def parse_news_page(self, response):
        ...
        date_published = response.css('div.article__header__datebar__date--original::text').get()
        headline = response.css('meta[property="og:title"]::attr(content)').get()
        author = response.css('span.contributor__meta__prefix~a::text').get()
        provenance = response.url
        last_modified = ''
        all_para_tags  =  response.css('span.abc__textblock.size--article>p::text').getall()
    ...
    def parse(self, response):
        ...
        links_in_page =  response.css("a.vice-card-hed__link::attr(href)").getall()
        next_page = response.css('a.pagination-page__next-link::attr(href)').get()
        for link in links_in_page:
            yield scrapy.Request(link, callback = self.parse_news_page,
            meta = {'query':query_params['q'][0]})
